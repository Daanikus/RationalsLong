...
/**
 * This tests the given Rational against a string representation of it. We also include some additional information to
 ...
 */
bool checkVal(const Rational &i, std::string val, std::string op, std::string fn) {
...
/**
 *
 * This tests the given Rational against another Rational instance. We also include some additional information to
 * assist with debugging.
 *
 ...
 * @return true if the integer is the same as the expected value
 */
bool checkVal(const Rational &i, const Rational &j, std::string op, std::string fn) {
    if (i != j) {
 ...
 */
bool simpleTestsRational(){

    bool success = true;

    Rational zero = Rational("0");
    Rational zero_default = Rational();
    Rational zero_copy = Rational(zero);
    Rational one = Rational("1");
    Rational one_copy = Rational(one);

    Rational negative_one = Rational("-1");
    Rational negative_one_copy = Rational(negative_one);
    Rational negative_one_unary = Rational(-one);


    /*
     * Some of you may not have seen the __func__ thing before. It's a macro that gets replaced with the name of
     * the current function. In this function it'll be the string "simpleTestsRational".
     */
...
bool logicTests(){
    ...
    Rational two_copy = Rational(two);
...
int main(void){
    ...
    success &= runTest(simpleTestsRational,  "Simple Tests (int)   ");
